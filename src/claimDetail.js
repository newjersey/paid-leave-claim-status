...
function executeOverride() {
  setupAnalytics(); // needed as is (meta)

  const { claimType, claimStatus, claimNotes } = extractData();

  removeOldHtml();
  addNewHtml();

  styleBody(); // needed as is (body, document)
  addFeedbackLink(); // needed as is (aside)
  updateIcon(); // needed as is (meta)
  makeMobileFriendly(); // needed as is (meta)
  logView(claimType, claimStatus, claimNotes); // needed as is (js log)
}
